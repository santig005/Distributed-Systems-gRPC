...
function loadOrdersFromFile() {
    try {
        if (fs.existsSync(ordersFilePath)) { // Verifica si el archivo existe
            ...
            console.warn("⚠️ Archivo orders.json no encontrado, iniciando con datos vacíos.");
...
export function createOrder({ userId, productIds, total }) {
    ...
    orders[id] = order; // Modifica la copia en memoria
    console.log("ℹ️ Nueva orden creada (en memoria):", id);

    // *** Llama a guardar después de modificar ***
    saveOrdersToFile();
